...
K_MAD_WEEKEND = 7.0
RECALIBRATE_WEEKS = 8
HIST_WINDOW_DAYS = 90 # Ventana de historial para iteración
...
# --- FUNCIONES DE UTILIDAD (main.py + features.py v7) ---
...
# --- FUNCIONES DE FEATURES (v7) ---
def add_time_parts(df):
    """Features de tiempo (versión v7)."""
    df_copy = df.copy()
    if 'ts' not in df_copy.columns: df_copy['ts'] = pd.to_datetime(df_copy.index) # Si 'ts' es el índice
    idx = df_copy["ts"]
    df_copy["hour"] = idx.dt.hour
    df_copy["dow"] = idx.dt.dayofweek
...
def dummies_and_reindex(df_row: pd.DataFrame, training_cols: list) -> pd.DataFrame:
    """Aplica dummies v7 (dow, month, hour)."""
    d = df_row.copy()
    d = pd.get_dummies(d, columns=['dow', 'month', 'hour'], drop_first=False)
    # Rellenar columnas faltantes
    for c in training_cols:
        if c not in d.columns:
            d[c] = 0.0 # Rellenar con float para consistencia
    return d.reindex(columns=training_cols, fill_value=0.0)
# --- FIN FUNCIONES FEATURES v7 ---
...
def compute_holiday_factors(df_hist, holidays_set, col_calls=TARGET_CALLS, col_tmo=TARGET_TMO):
    ...
    if 'dow' not in df_hist.columns: dfh = add_time_parts(df_hist.copy())
...
def main(horizonte_dias):
    print("="*60); print(f"INICIANDO PIPELINE DE INFERENCIA (v_main 30 - MLP v7 + Iterativo + Post-Proceso)"); print(f"Zona Horaria: {TZ} | Horizonte: {horizonte_dias} días"); print("="*60) # v30
    ...
    df_hosting_processed_for_tmo = add_time_parts(df_hist_merged_for_tmo) # v7 features
    ...
    df_hosting_processed_for_adjustments = add_time_parts(df_hist_for_adjustments) # v7 features
    ...
    # --- 5. Pipeline Llamadas (MLP v7 - Predicción Iterativa) ---
    print("\n--- Fase 5: Pipeline de Llamadas (Planificador MLP v7 - Iterativo) ---")
    
    # --- AJUSTE v30: LÓGICA ITERATIVA (como inferencia_core.py) ---
    hist_window_start = last_hist_ts - pd.Timedelta(days=HIST_WINDOW_DAYS)
    # Usar el historial procesado v7
    df_hist_window = df_hosting_processed_for_adjustments[df_hosting_processed_for_adjustments['ts'] >= hist_window_start].copy()
    # Asegurar que el historial tenga lags/MAs (necesarios para la primera predicción)
    df_hist_window = add_lags_mas(df_hist_window, TARGET_CALLS)
    
    predictions_planner = []
    print(f"  Iniciando predicción iterativa para {len(df_future)} horas...")
    
    df_future_with_preds = df_future.copy()
    
    for ts_futuro in df_future['ts']:
        df_step = pd.DataFrame({'ts': [ts_futuro]})
        df_full = pd.concat([df_hist_window, df_step], ignore_index=True) 
        
        # Generar features v7 (Lags, MAs, Time parts)
        df_full = add_lags_mas(df_full, TARGET_CALLS)
        df_full = add_time_parts(df_full) # v7 features
        
        X_step_df = df_full.iloc[-1:]
        
        # Aplicar dummies v7 (dow, month, hour)
        X_step_with_dummies = dummies_and_reindex(X_step_df, cols_planner)
        # Reindexar con columnas de entrenamiento v7
        X_step_reindexed = X_step_with_dummies.reindex(columns=cols_planner, fill_value=0)
        ...
        # (El historial debe tener las mismas columnas que el original)
    ...
    print("  [OK] Predicciones BASE iterativas (MLP v7) generadas.")
    # --- Fin Ajuste Iterativo ---

    # --- 6. Pipeline TMO (MLP v7 - Predicción Directa) ---
    print("\n--- Fase 6: Pipeline de TMO (Analista de Operaciones MLP v7) ---")
    ...
    df_tmo_features_future = add_time_parts(df_tmo_features_future) # Añadir dow, month, hour, etc.
    
    for col in seed_cols: df_tmo_features_future[col] = last_tmo_data.get(col, 0)
         
    X_tmo_base = df_tmo_features_future.copy()
    ...
    df_tmo_features_future_with_clima = dummies_and_reindex(df_tmo_features_future_with_clima, cols_tmo) # Dummies para TMO
    ...
    df_future_with_risk_features = dummies_and_reindex(df_future.copy(), cols_risk) # Dummies para Risk
